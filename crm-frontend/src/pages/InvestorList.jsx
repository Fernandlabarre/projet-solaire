...
export default function InvestorList() {
  ...
  const handleSave = async () => {
    try {
      if (editId) {
        await api.put(`/investors/${editId}`, form);
      } else {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Supprimer cet investisseur ?")) return;
    await api.delete(`/investors/${id}`);
    setInvestors((list) => list.filter((inv) => inv.id !== id));
  };

  // Recherche
  const filteredRows = investors.filter((inv) =>
    Object.values(inv).join(" ").toLowerCase().includes(search.toLowerCase())
  );

  const columns = [
    { field: "id", headerName: "ID", width: 80 },
    { field: "name", headerName: "Nom", flex: 1, minWidth: 160 },
    { field: "company", headerName: "Entreprise", flex: 1, minWidth: 160 },
    { field: "email", headerName: "Email", flex: 1, minWidth: 180 },
    { field: "phone", headerName: "Téléphone", flex: 1, minWidth: 140 },
          ...
          >
            <Box>
              ...
              <Typography color="text.secondary" fontSize={15}>
                Gestion de tous les investisseurs
              </Typography>
            </Box>
            <Button
              variant="contained"
